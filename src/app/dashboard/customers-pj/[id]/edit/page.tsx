...
function formatJapanesePhone(value: string): string {
  ...
  if (!digits.startsWith('0')) return value;
  const n = digits.length;
  // Celular 11 dígitos: 080-1234-5678
  if (n <= 11 && n > 7 && ['08','09','07','05'].includes(digits.slice(0,2))) {
  ...
  const fetchCustomer = async () => {
    try {
      ...
      setFormData({
        ...
        billingCycle: (c as any).billingCycle || 'MONTHLY',
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input type="tel" required value={formData.phone}
                    onChange={(e) => setFormData({ ...formData, phone: formatJapanesePhone(e.target.value) })}
                    className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500" />
                </div>
              </div>
              <div>
                ...
                <input type="tel" value={formData.phoneAlt}
                  onChange={(e) => setFormData({ ...formData, phoneAlt: formatJapanesePhone(e.target.value) })}
                  className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500" />
              </div>
            ...
            {formData.allowedPaymentMethods.includes('INVOICE') && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div className="space-y-4">
                  ...
                  {/* Ciclo de Fechamento */}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Ciclo de Fechamento / 締めサイクル</label>
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        { value: 'WEEKLY', label: '週次', labelPt: 'Semanal' },
                        { value: 'BIWEEKLY', label: '隔週', labelPt: 'Quinzenal' },
                        { value: 'MONTHLY', label: '月次', labelPt: 'Mensal' },
                      ].map(cycle => (
                        <label key={cycle.value} className={`flex flex-col items-center p-2.5 border-2 rounded-lg cursor-pointer transition-all text-center ${
                          formData.billingCycle === cycle.value ? 'border-red-500 bg-red-50' : 'border-gray-200 hover:border-gray-300'
                        }`}>
                          <input type="radio" name="billingCycle" value={cycle.value}
                            checked={formData.billingCycle === cycle.value}
                            onChange={(e) => setFormData({ ...formData, billingCycle: e.target.value })}
                            className="sr-only" />
                          <span className="font-medium text-sm text-gray-900">{cycle.label}</span>
                          <span className="text-[10px] text-gray-500">{cycle.labelPt}</span>
                        </label>
                      ))}
                    </div>
                  </div>

                  {/* Dia de Fechamento - só para mensal */}
                  {formData.billingCycle === 'MONTHLY' && (
                    <div>
                      ...
                      <select value={formData.billingClosingDay} onChange={(e) => setFormData({ ...formData, billingClosingDay: e.target.value })}
                        className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                        <option value="31">月末 / Fim do Mês</option>
                        <option value="5">5日 / Dia 5</option>
                        <option value="10">10日 / Dia 10</option>
                        <option value="15">15日 / Dia 15</option>
                        <option value="20">20日 / Dia 20</option>
                        <option value="25">25日 / Dia 25</option>
                      </select>
                    </div>
                  )}
                  ...
                  {formData.billingCycle === 'BIWEEKLY' && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                      <p className="text-xs text-blue-800">
                        📅 Fechamento quinzenal: dia 15 e último dia do mês.<br/>
                        隔週締め：15日と月末に締め切り
                      </p>
                    </div>
                  )}
                  ...
                  {formData.billingCycle === 'WEEKLY' && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                      <p className="text-xs text-blue-800">
                        📅 Fechamento semanal: toda semana (sábado).<br/>
                        週次締め：毎週土曜日に締め切り
                      </p>
                    </div>
                  )}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Dia de Pagamento / 支払日</label>
                    <select value={formData.billingDueDay} onChange={(e) => setFormData({ ...formData, billingDueDay: e.target.value })}
                      className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                      <option value="10">10日 / Dia 10</option>
                      <option value="15">15日 / Dia 15</option>
                      <option value="31">月末 / Dia 31 (Fim do mês)</option>
                    </select>
                  </div>

                  {/* Prazo de Pagamento */}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Prazo de Pagamento / 支払サイト</label>
                    <select value={formData.paymentTerms} onChange={(e) => setFormData({ ...formData, paymentTerms: e.target.value })}
                      className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                      <option value="15">15日 / 15 dias</option>
                      <option value="30">30日 / 30 dias</option>
                      <option value="45">45日 / 45 dias</option>
                      <option value="60">60日 / 60 dias</option>
                    </select>
                  </div>
                </div>
              </div>
