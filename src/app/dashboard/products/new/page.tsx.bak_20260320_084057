...
import { ArrowLeft, Calculator, Percent, Barcode, Package } from 'lucide-react';
...
export default function NewProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('製品が作成されました / Produto criado com sucesso!');
      router.push('/dashboard/products');
    } catch (error) {
      console.error('Erro ao criar produto:', error);
      alert('製品の作成に失敗しました / Erro ao criar produto');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-4 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">新規製品 / Novo Produto</h1>
            <p className="text-gray-500 mt-1">製品情報を入力してください</p>
          </div>
        </div>
      </div>
